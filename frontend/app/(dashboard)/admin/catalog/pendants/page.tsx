...
export default function PendantCatalog() {
    const [searchTerm, setSearchTerm] = useState("")
    const [filterCategory, setFilterCategory] = useState("All Pendants")
    ...
    const { data: jewelryData, isLoading: isJewelryLoading } = useGetJewelryQuery({
        ...
        category: filterCategory === 'All Pendants' ? undefined : filterCategory,
        sort: '-createdAt'
    })
    ...
    const pendantCategories = allCategories.filter((c: any) =>
        c.level === 0 && (c.name.toLowerCase().includes("pendant") || c.name.toLowerCase().includes("jewelry"))
    )
    ...
    const [newItem, setNewItem] = useState({
        ...
        dimensions: "",
        totalWeight: "",
        bailType: "", // Specific to Pendant
        chainIncluded: "No", // Specific to Pendant
        slug: "",
        metaTitle: "",
        metaDescription: "",
        keywords: "",
    ...
    useEffect(() => {
        if (pendantCategories.length > 0 && !newItem.category) {
            setNewItem(prev => ({ ...prev, category: pendantCategories[0]._id }))
        }
    }, [pendantCategories])
    ...
    // Filter displayed items to only show pendants
    const jewelry = (jewelryData?.data || []).filter((item: any) =>
        item.category?.name?.toLowerCase().includes('pendant') ||
        item.categoryName?.toLowerCase().includes('pendant') ||
        (item.title && item.title.toLowerCase().includes('pendant'))
    );
    ...
    const handleDelete = async (id: string) => {
        if (confirm("Are you sure you want to remove this pendant?")) {
            try {
    ...
    const handleEdit = (item: any) => {
        ...
        setNewItem({
            ...
            bailType: item.bailType || "",
            chainIncluded: item.chainIncluded || "No",
            slug: item.slug || "",
    ...
    const handleAddItem = async (e: React.FormEvent) => {
        ...
        try {
            ...
            formData.append('bailType', newItem.bailType)
            formData.append('chainIncluded', newItem.chainIncluded)
            ...
            if (editingItem) {
                ...
                toast.success("Pendant updated successfully")
            } else {
                await createJewelry(formData).unwrap()
                toast.success("Pendant added successfully")
            }

            setIsModalOpen(false)
            setEditingItem(null)
            setNewItem({
                name: "", category: pendantCategories[0]?._id || "", subcategory: "",
                collectionName: "Signature", metalType: "14K White Gold",
                price: "", stock: "", description: "", gemstoneType: "", dimensions: "", totalWeight: "",
                bailType: "", chainIncluded: "No",
    ...
    const handleCsvUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            formData.append('productType', 'pendant')
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6">
                <div>
                    ...
                    <h1 className="text-3xl sm:text-4xl font-serif text-slate-900 leading-tight">Pendant Collection</h1>
                    <p className="text-slate-500 mt-1 max-w-xl">Curate your pendants. Manage chain inclusions and bail types.</p>
                </div>
                <div className="flex flex-wrap items-center gap-3">
                    ...
                    >
                        <Plus className="w-4 h-4 group-hover:rotate-90 transition-transform duration-300" /> New Pendant
                    </button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                    { label: 'Total Pendants', value: jewelry.length, unit: 'Items', icon: ShoppingBag },
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col lg:flex-row items-center gap-4">
                <div className="relative flex-1 w-full">
                    ...
                    <input
                        type="text"
                        placeholder="Search pendants by name, style or SKU..."
                        ...
                        <table className="w-full text-left min-w-[900px]">
                            <thead>
                                <tr className="bg-slate-50 border-b border-slate-100">
                                    <th className="px-10 py-7 text-[10px] uppercase tracking-[0.2em] text-slate-400 font-bold">Pendant</th>
                                    ...
                                    <tr key={item._id} className="hover:bg-slate-50/50 transition-colors group">
                                        ...
                                        <td className="px-8 py-6">
                                            <div className="flex flex-col gap-1">
                                                <span className="text-[10px] font-bold text-[#163E3E] uppercase tracking-widest">{item.category?.name || 'Pendant'}</span>
                                ...
                                {jewelry.length === 0 && !isJewelryLoading && (
                                    <tr>
                                        <td colSpan={5} className="px-10 py-32 text-center text-slate-400">
                                            ...
                                            <p className="text-[11px] font-bold uppercase tracking-widest mt-2">No pendants found in catalog</p>
                                        </td>
                                    </tr>
                                )}
                            </tbody>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-500">
                    <div className="bg-white w-full max-w-4xl rounded-[48px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-500">
                        <div className="px-12 py-10 border-b border-slate-100 flex items-center justify-between bg-slate-50/30">
                            <div>
                                <h2 className="text-3xl font-serif text-slate-900">Add Pendant</h2>
                        ...
                        <form onSubmit={handleAddItem} className="p-12">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-12 gap-12">
                                            ...
                                            <div className="md:col-span-8 space-y-8">
                                                <div className="space-y-4">
                                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest pl-1">Pendant Name</label>
                                                    <input required type="text" className="w-full px-7 py-5 bg-slate-50 border-none rounded-[24px] text-sm font-bold text-slate-900 outline-none shadow-sm focus:ring-2 focus:ring-[#163E3E]/10" placeholder="e.g. Diamond Heart Pendant" value={newItem.name} onChange={(e) => setNewItem({ ...newItem, name: e.target.value })} />
                                                </div>
                                ...
                                {modalTab === 'specs' && (
                                    <div className="space-y-10 animate-in fade-in slide-in-from-right-4 duration-500">
                                        <div className="grid grid-cols-2 gap-12">
                                            <div className="space-y-8">
                                                <div className="space-y-4">
                                                    ...
                                                    <select className="w-full px-7 py-5 bg-slate-50 border-none rounded-[24px] text-sm font-bold text-slate-900 outline-none shadow-sm focus:ring-2 focus:ring-[#163E3E]/10 cursor-pointer" value={newItem.category} onChange={(e) => setNewItem({ ...newItem, category: e.target.value, subcategory: "" })}>
                                                        {pendantCategories.map((c: any) => <option key={c._id} value={c._id}>{c.name}</option>)}
                                                    </select>
                                                </div>
                                                <div className="space-y-4">
                                            ...
                                            <div className="space-y-8">
                                                <div className="space-y-4">
                                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest pl-1">Bail Type</label>
                                                    <input type="text" className="w-full px-7 py-5 bg-slate-50 border-none rounded-[24px] text-sm font-bold text-slate-900 outline-none shadow-sm focus:ring-2 focus:ring-[#163E3E]/10" placeholder="e.g. Standard, Rabbit Ear" value={newItem.bailType} onChange={(e) => setNewItem({ ...newItem, bailType: e.target.value })} />
                                                </div>
                                                <div className="space-y-4">
                                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest pl-1">Chain Included</label>
                                                    <select className="w-full px-7 py-5 bg-slate-50 border-none rounded-[24px] text-sm font-bold text-slate-900 outline-none shadow-sm focus:ring-2 focus:ring-[#163E3E]/10" value={newItem.chainIncluded} onChange={(e) => setNewItem({ ...newItem, chainIncluded: e.target.value })}>
                                                        <option>No</option>
                                                        <option>Yes</option>
                                                    </select>
                                                </div>
                                ...
                                {modalTab === 'seo' && (
                                    <div className="space-y-10 animate-in fade-in slide-in-from-right-4 duration-500">
                                        <div className="space-y-4">
                                            ...
                                            <input type="text" className="w-full px-7 py-5 bg-slate-50 border-none rounded-[24px] text-sm font-bold text-slate-900 outline-none shadow-sm focus:ring-2 focus:ring-[#163E3E]/10" placeholder="pendant-name-slug" value={newItem.slug} onChange={(e) => setNewItem({ ...newItem, slug: e.target.value })} />
                                        </div>
                                        <div className="space-y-4">
                            ...
                            <div className="flex items-center gap-6 pt-12 mt-12 border-t border-slate-100">
                                ...
                                <button type="submit" className="flex-1 px-12 py-5 bg-[#163E3E] text-white rounded-[32px] text-[11px] font-bold uppercase tracking-[0.2em] hover:bg-black transition-all shadow-2xl shadow-[#163E3E]/30 active:scale-95 transform">Save Pendant</button>
                            </div>
                        </form>
                    </div>
                </div>
                                    ...
                                    <div>
                                        ...
                                        <div className="space-y-3">
                                            {[
                                                ...
                                                { label: 'Bail Type', value: viewingItem.bailType || 'N/A' },
                                                { label: 'Chain Included', value: viewingItem.chainIncluded || 'No' },
