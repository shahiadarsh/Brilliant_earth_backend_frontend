...
export default function EarringCatalog() {
    const [searchTerm, setSearchTerm] = useState("")
    const [filterCategory, setFilterCategory] = useState("All Earrings")
    ...
    const { data: jewelryData, isLoading: isJewelryLoading } = useGetJewelryQuery({
        ...
        category: filterCategory === 'All Earrings' ? undefined : filterCategory,
        sort: '-createdAt'
    })
    ...
    const earringCategories = allCategories.filter((c: any) =>
        c.level === 0 && (c.name.toLowerCase().includes("earring") || c.name.toLowerCase().includes("jewelry"))
    )
    ...
    const [newItem, setNewItem] = useState({
        ...
        dimensions: "",
        totalWeight: "",
        backingType: "", // Specific to Earrings
        dropLength: "", // Specific to Earrings
        slug: "",
        metaTitle: "",
        metaDescription: "",
        keywords: "",
    ...
    useEffect(() => {
        if (earringCategories.length > 0 && !newItem.category) {
            setNewItem(prev => ({ ...prev, category: earringCategories[0]._id }))
        }
    }, [earringCategories])
    ...
    // Filter displayed items to only show earrings
    const jewelry = (jewelryData?.data || []).filter((item: any) =>
        item.category?.name?.toLowerCase().includes('earring') ||
        item.categoryName?.toLowerCase().includes('earring') ||
        (item.title && item.title.toLowerCase().includes('earring'))
    );
    ...
    const handleDelete = async (id: string) => {
        if (confirm("Are you sure you want to remove these earrings?")) {
            try {
    ...
    const handleEdit = (item: any) => {
        ...
        setNewItem({
            ...
            backingType: item.backingType || "",
            dropLength: item.dropLength || "",
            slug: item.slug || "",
    ...
    const handleAddItem = async (e: React.FormEvent) => {
        ...
        try {
            ...
            formData.append('backingType', newItem.backingType)
            formData.append('dropLength', newItem.dropLength)
            ...
            if (editingItem) {
                ...
                toast.success("Earrings updated successfully")
            } else {
                await createJewelry(formData).unwrap()
                toast.success("Earrings added successfully")
            }

            setIsModalOpen(false)
            setEditingItem(null)
            setNewItem({
                name: "", category: earringCategories[0]?._id || "", subcategory: "",
                collectionName: "Signature", metalType: "14K White Gold",
                price: "", stock: "", description: "", gemstoneType: "", dimensions: "", totalWeight: "",
                backingType: "", dropLength: "",
    ...
    const handleCsvUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            formData.append('productType', 'earring')
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6">
                <div>
                    ...
                    <h1 className="text-3xl sm:text-4xl font-serif text-slate-900 leading-tight">Earring Collection</h1>
                    <p className="text-slate-500 mt-1 max-w-xl">Curate fine earrings. Manage types from studs to drops.</p>
                </div>
                <div className="flex flex-wrap items-center gap-3">
                    ...
                    >
                        <Plus className="w-4 h-4 group-hover:rotate-90 transition-transform duration-300" /> New Earring
                    </button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                    { label: 'Total Earrings', value: jewelry.length, unit: 'Items', icon: ShoppingBag },
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col lg:flex-row items-center gap-4">
                <div className="relative flex-1 w-full">
                    ...
                    <input
                        type="text"
                        placeholder="Search earrings by name, style or SKU..."
                        ...
                        <table className="w-full text-left min-w-[900px]">
                            <thead>
                                <tr className="bg-slate-50 border-b border-slate-100">
                                    <th className="px-10 py-7 text-[10px] uppercase tracking-[0.2em] text-slate-400 font-bold">Earring</th>
                                    ...
                                    <tr key={item._id} className="hover:bg-slate-50/50 transition-colors group">
                                        ...
                                        <td className="px-8 py-6">
                                            <div className="flex flex-col gap-1">
                                                <span className="text-[10px] font-bold text-[#163E3E] uppercase tracking-widest">{item.category?.name || 'Earring'}</span>
                                ...
                                {jewelry.length === 0 && !isJewelryLoading && (
                                    <tr>
                                        <td colSpan={5} className="px-10 py-32 text-center text-slate-400">
                                            ...
                                            <p className="text-[11px] font-bold uppercase tracking-widest mt-2">No earrings found in catalog</p>
                                        </td>
                                    </tr>
                                )}
                            </tbody>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-500">
                    <div className="bg-white w-full max-w-4xl rounded-[48px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-500">
                        <div className="px-12 py-10 border-b border-slate-100 flex items-center justify-between bg-slate-50/30">
                            <div>
                                <h2 className="text-3xl font-serif text-slate-900">Add Earring</h2>
                        ...
                        <form onSubmit={handleAddItem} className="p-12">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-12 gap-12">
                                            ...
                                            <div className="md:col-span-8 space-y-8">
                                                <div className="space-y-4">
                                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest pl-1">Earring Name</label>
                                                    <input required type="text" className="w-full px-7 py-5 bg-slate-50 border-none rounded-[24px] text-sm font-bold text-slate-900 outline-none shadow-sm focus:ring-2 focus:ring-[#163E3E]/10" placeholder="e.g. Diamond Stud Earrings" value={newItem.name} onChange={(e) => setNewItem({ ...newItem, name: e.target.value })} />
                                                </div>
                                ...
                                {modalTab === 'specs' && (
                                    <div className="space-y-10 animate-in fade-in slide-in-from-right-4 duration-500">
                                        <div className="grid grid-cols-2 gap-12">
                                            <div className="space-y-8">
                                                <div className="space-y-4">
                                                    ...
                                                    <select className="w-full px-7 py-5 bg-slate-50 border-none rounded-[24px] text-sm font-bold text-slate-900 outline-none shadow-sm focus:ring-2 focus:ring-[#163E3E]/10 cursor-pointer" value={newItem.category} onChange={(e) => setNewItem({ ...newItem, category: e.target.value, subcategory: "" })}>
                                                        {earringCategories.map((c: any) => <option key={c._id} value={c._id}>{c.name}</option>)}
                                                    </select>
                                                </div>
                                                <div className="space-y-4">
                                            ...
                                            <div className="space-y-8">
                                                <div className="space-y-4">
                                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest pl-1">Backing Type</label>
                                                    <input type="text" className="w-full px-7 py-5 bg-slate-50 border-none rounded-[24px] text-sm font-bold text-slate-900 outline-none shadow-sm focus:ring-2 focus:ring-[#163E3E]/10" placeholder="e.g. Push Back, Screw Back" value={newItem.backingType} onChange={(e) => setNewItem({ ...newItem, backingType: e.target.value })} />
                                                </div>
                                                <div className="space-y-4">
                                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest pl-1">Drop Length</label>
                                                    <input type="text" className="w-full px-7 py-5 bg-slate-50 border-none rounded-[24px] text-sm font-bold text-slate-900 outline-none shadow-sm focus:ring-2 focus:ring-[#163E3E]/10" placeholder="e.g. 1.5 inches" value={newItem.dropLength} onChange={(e) => setNewItem({ ...newItem, dropLength: e.target.value })} />
                                                </div>
                                                <div className="space-y-4">
                                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest pl-1">Gemstones</label>
                                                    <input type="text" className="w-full px-7 py-5 bg-slate-50 border-none rounded-[24px] text-sm font-bold text-slate-900 outline-none shadow-sm focus:ring-2 focus:ring-[#163E3E]/10" placeholder="e.g. Diamond, Pearl" value={newItem.gemstoneType} onChange={(e) => setNewItem({ ...newItem, gemstoneType: e.target.value })} />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                {modalTab === 'seo' && (
                                    <div className="space-y-10 animate-in fade-in slide-in-from-right-4 duration-500">
                                        <div className="space-y-4">
                                            ...
                                            <input type="text" className="w-full px-7 py-5 bg-slate-50 border-none rounded-[24px] text-sm font-bold text-slate-900 outline-none shadow-sm focus:ring-2 focus:ring-[#163E3E]/10" placeholder="earring-name-slug" value={newItem.slug} onChange={(e) => setNewItem({ ...newItem, slug: e.target.value })} />
                                        </div>
                                        <div className="space-y-4">
                            ...
                            <div className="flex items-center gap-6 pt-12 mt-12 border-t border-slate-100">
                                ...
                                <button type="submit" className="flex-1 px-12 py-5 bg-[#163E3E] text-white rounded-[32px] text-[11px] font-bold uppercase tracking-[0.2em] hover:bg-black transition-all shadow-2xl shadow-[#163E3E]/30 active:scale-95 transform">Save Earring</button>
                            </div>
                        </form>
                    </div>
                </div>
                                    ...
                                    <div>
                                        ...
                                        <div className="space-y-3">
                                            {[
                                                ...
                                                { label: 'Backing Type', value: viewingItem.backingType || 'N/A' },
                                                { label: 'Drop Length', value: viewingItem.dropLength || 'N/A' },
