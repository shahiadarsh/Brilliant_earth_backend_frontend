...
export default function NecklaceCatalog() {
    const [searchTerm, setSearchTerm] = useState("")
    const [filterCategory, setFilterCategory] = useState("All Necklaces")
    ...
    // API Hooks - Reusing jewelry API for now as requested (frontend focus)
    // In a real app, this might be filtered by type 'necklace' if backend supported it natively in one endpoint or a specific endpoint
    const { data: jewelryData, isLoading: isJewelryLoading } = useGetJewelryQuery({
        search: searchTerm || undefined,
        category: filterCategory === 'All Necklaces' ? undefined : filterCategory,
        sort: '-createdAt'
    })
    ...
    // Filter for Necklace related categories if they exist, otherwise fallback to general jewelry
    const necklaceCategories = allCategories.filter((c: any) =>
        c.level === 0 && (c.name.toLowerCase().includes("necklace") || c.name.toLowerCase().includes("jewelry"))
    )
    ...
    const [newItem, setNewItem] = useState({
        ...
        dimensions: "",
        totalWeight: "",
        chainLength: "", // Specific to Necklace
        claspType: "", // Specific to Necklace
        slug: "",
        metaTitle: "",
        metaDescription: "",
        keywords: "",
    ...
    useEffect(() => {
        if (necklaceCategories.length > 0 && !newItem.category) {
            setNewItem(prev => ({ ...prev, category: necklaceCategories[0]._id }))
        }
    }, [necklaceCategories])
    ...
    // Filter displayed items to only show necklaces
    const jewelry = (jewelryData?.data || []).filter((item: any) =>
        item.category?.name?.toLowerCase().includes('necklace') ||
        item.categoryName?.toLowerCase().includes('necklace') ||
        (item.title && item.title.toLowerCase().includes('necklace'))
    );
    ...
    const handleDelete = async (id: string) => {
        if (confirm("Are you sure you want to remove this necklace?")) {
            try {
    ...
    const handleAddItem = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Note: chainLength and claspType might not be saved by backend yet, but added to formData
            formData.append('chainLength', newItem.chainLength)
            ...
            if (editingItem) {
                ...
                toast.success("Necklace added successfully")
            }

            setIsModalOpen(false)
            setEditingItem(null)
            setNewItem({
                name: "", category: necklaceCategories[0]?._id || "", subcategory: "",
                collectionName: "Signature", metalType: "14K White Gold",
                price: "", stock: "", description: "", gemstoneType: "", dimensions: "", totalWeight: "",
                chainLength: "", claspType: "",
    ...
    const handleCsvUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            formData.append('productType', 'necklace')
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6">
                <div>
                    ...
                    <h1 className="text-3xl sm:text-4xl font-serif text-slate-900 leading-tight">Necklace Collection</h1>
                    <p className="text-slate-500 mt-1 max-w-xl">Curate your fine necklaces. Manage properties like chain length and clasp type.</p>
                </div>
                <div className="flex flex-wrap items-center gap-3">
                    ...
                    >
                        <Plus className="w-4 h-4 group-hover:rotate-90 transition-transform duration-300" /> New Necklace
                    </button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                    { label: 'Total Necklaces', value: jewelry.length, unit: 'Items', icon: ShoppingBag },
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col lg:flex-row items-center gap-4">
                <div className="relative flex-1 w-full">
                    ...
                    <input
                        type="text"
                        placeholder="Search necklaces by name, style or SKU..."
                        ...
                        <table className="w-full text-left min-w-[900px]">
                            <thead>
                                <tr className="bg-slate-50 border-b border-slate-100">
                                    <th className="px-10 py-7 text-[10px] uppercase tracking-[0.2em] text-slate-400 font-bold">Necklace</th>
                                    ...
                                    <tr key={item._id} className="hover:bg-slate-50/50 transition-colors group">
                                        ...
                                        <td className="px-8 py-6">
                                            <div className="flex flex-col gap-1">
                                                <span className="text-[10px] font-bold text-[#163E3E] uppercase tracking-widest">{item.category?.name || 'Necklace'}</span>
                                ...
                                {jewelry.length === 0 && !isJewelryLoading && (
                                    <tr>
                                        <td colSpan={5} className="px-10 py-32 text-center text-slate-400">
                                            ...
                                            <p className="text-[11px] font-bold uppercase tracking-widest mt-2">No necklaces found in catalog</p>
                                        </td>
                                    </tr>
                                )}
                            </tbody>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-500">
                    <div className="bg-white w-full max-w-4xl rounded-[24px] md:rounded-[48px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-500 my-4 md:my-0 h-[90vh] md:h-auto flex flex-col md:block">
                        <div className="px-6 py-6 md:px-12 md:py-10 border-b border-slate-100 flex items-center justify-between bg-slate-50/30 shrink-0">
                            <div>
                                <h2 className="text-2xl md:text-3xl font-serif text-slate-900">Add Necklace</h2>
                            ...
                            <button onClick={() => setIsModalOpen(false)} className="p-3 md:p-4 hover:bg-white rounded-3xl transition-all shadow-sm active:scale-90 border border-transparent hover:border-slate-100">
                                <X className="w-6 h-6 md:w-7 md:h-7 text-slate-400" />
                            </button>
                        </div>

                        {/* Modal Navigation */}
                        <div className="flex items-center px-4 md:px-12 border-b border-slate-50 bg-white shadow-sm overflow-x-auto no-scrollbar shrink-0">
                            {[
                                ...
                                <button
                                    ...
                                    className={`px-4 md:px-10 py-5 md:py-6 text-[10px] md:text-[11px] font-bold uppercase tracking-[0.2em] transition-all relative whitespace-nowrap
                        ...
                        <form onSubmit={handleAddItem} className="p-6 md:p-12 overflow-y-auto flex-1 md:flex-none">
                            <div className="max-h-[none] md:max-h-[55vh] overflow-y-auto pr-0 md:pr-6 custom-scrollbar pb-20 md:pb-0">
                                {modalTab === 'details' && (
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-12 gap-12">
                                            ...
                                            <div className="md:col-span-4">
                                                ...
                                                >
                                                    ...
                                                    <div className="text-center px-6">
                                                        <p className="text-[11px] font-bold text-slate-800 uppercase tracking-widest">Upload Images</p>
                                                    </div>
                                                </div>
                                            ...
                                            <div className="md:col-span-8 space-y-8">
                                                <div className="space-y-4">
                                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest pl-1">Necklace Name</label>
                                                    <input required type="text" className="w-full px-7 py-5 bg-slate-50 border-none rounded-[24px] text-sm font-bold text-slate-900 outline-none shadow-sm focus:ring-2 focus:ring-[#163E3E]/10" placeholder="e.g. Diamond Pendant Necklace" value={newItem.name} onChange={(e) => setNewItem({ ...newItem, name: e.target.value })} />
                                                </div>
                                ...
                                {modalTab === 'specs' && (
                                    <div className="space-y-10 animate-in fade-in slide-in-from-right-4 duration-500">
                                        <div className="grid grid-cols-2 gap-12">
                                            <div className="space-y-8">
                                                <div className="space-y-4">
                                                    ...
                                                    <select className="w-full px-7 py-5 bg-slate-50 border-none rounded-[24px] text-sm font-bold text-slate-900 outline-none shadow-sm focus:ring-2 focus:ring-[#163E3E]/10 cursor-pointer" value={newItem.category} onChange={(e) => setNewItem({ ...newItem, category: e.target.value, subcategory: "" })}>
                                                        {necklaceCategories.map((c: any) => <option key={c._id} value={c._id}>{c.name}</option>)}
                                                    </select>
                                                </div>
                                                <div className="space-y-4">
                                            ...
                                            <div className="space-y-8">
                                                <div className="space-y-4">
                                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest pl-1">Chain Length</label>
                                                    <input type="text" className="w-full px-7 py-5 bg-slate-50 border-none rounded-[24px] text-sm font-bold text-slate-900 outline-none shadow-sm focus:ring-2 focus:ring-[#163E3E]/10" placeholder="e.g. 18 inches" value={newItem.chainLength} onChange={(e) => setNewItem({ ...newItem, chainLength: e.target.value })} />
                                                </div>
                                                <div className="space-y-4">
                                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest pl-1">Clasp Type</label>
                                                    <input type="text" className="w-full px-7 py-5 bg-slate-50 border-none rounded-[24px] text-sm font-bold text-slate-900 outline-none shadow-sm focus:ring-2 focus:ring-[#163E3E]/10" placeholder="e.g. Lobster Claw" value={newItem.claspType} onChange={(e) => setNewItem({ ...newItem, claspType: e.target.value })} />
                                                </div>
                                                <div className="space-y-4">
                                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest pl-1">Gemstones</label>
                                                    <input type="text" className="w-full px-7 py-5 bg-slate-50 border-none rounded-[24px] text-sm font-bold text-slate-900 outline-none shadow-sm focus:ring-2 focus:ring-[#163E3E]/10" placeholder="e.g. Diamond, Sapphire" value={newItem.gemstoneType} onChange={(e) => setNewItem({ ...newItem, gemstoneType: e.target.value })} />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                {modalTab === 'seo' && (
                                    <div className="space-y-10 animate-in fade-in slide-in-from-right-4 duration-500">
                                        <div className="space-y-4">
                                            ...
                                            <input type="text" className="w-full px-7 py-5 bg-slate-50 border-none rounded-[24px] text-sm font-bold text-slate-900 outline-none shadow-sm focus:ring-2 focus:ring-[#163E3E]/10" placeholder="necklace-name-slug" value={newItem.slug} onChange={(e) => setNewItem({ ...newItem, slug: e.target.value })} />
                                        </div>
                                        <div className="space-y-4">
                                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest pl-1">Meta Title</label>
                                            <input type="text" className="w-full px-7 py-5 bg-slate-50 border-none rounded-[24px] text-sm font-bold text-slate-900 outline-none shadow-sm focus:ring-2 focus:ring-[#163E3E]/10" placeholder="SEO Title" value={newItem.metaTitle} onChange={(e) => setNewItem({ ...newItem, metaTitle: e.target.value })} />
                                        </div>
                                        <div className="space-y-4">
                            ...
                            <div className="flex items-center gap-6 pt-12 mt-12 border-t border-slate-100">
                                <button type="button" onClick={() => setIsModalOpen(false)} className="px-12 py-5 bg-white border border-slate-200 text-slate-400 rounded-[32px] text-[11px] font-bold uppercase tracking-[0.2em] hover:text-slate-600 hover:bg-slate-50 transition-all active:scale-95">Cancel</button>
                                <button type="submit" className="flex-1 px-12 py-5 bg-[#163E3E] text-white rounded-[32px] text-[11px] font-bold uppercase tracking-[0.2em] hover:bg-black transition-all shadow-2xl shadow-[#163E3E]/30 active:scale-95 transform">Save Necklace</button>
                            </div>
                        </form>
                    </div>
                </div>
                                    ...
                                    <div>
                                        ...
                                        <div className="space-y-3">
                                            {[
                                                ...
                                                { label: 'Chain Length', value: viewingItem.chainLength || 'N/A' },
